...
$res = mysqli_query($mysqli, "select * from normal_bookingtb where status='Unbooked'");

if(isset($_POST['log'])){
    $res = mysqli_query($mysqli, "select * from normal_bookingtb");
    $pdf = new FPDF();
    $pdf->AddPage();
    $pdf->SetFont('Arial','B',20);
    $pdf->Cell(55,20,'');
    $pdf->Cell(10,20,'Normal Booking Log Report');
    $pdf->Ln();
    ...
    while($row = mysqli_fetch_array($res)){
        $pdf->Cell(10,8,$row['booking_id'],1);
        ...
        $pdf->Cell(25,8,$row['vehical_no'],1);
        $pdf->Cell(20,8,$row['trip_date'],1);
        $pdf->Cell(15,8,$row['status'],1);
        $pdf->Cell(15,8,$row['payment_st'],1);
        $pdf->Ln();
    }
...
<html>
    ...
    <?php }?>
    <div class="container">
        <h2 style="margin-top: 20px;">Normal Bookings</h2>
    ...
    while($row = mysqli_fetch_array($res)){
        ...
        echo "<td><a href='normal_b_client_form.php?logcount=".$logcount."&booking_id=".$row['booking_id']."&username=".$username."'><button class='btn btn-outline-light btn-sm' type='submit'>View</button></a></td>";
        echo "</tr>";
        echo"</tbody>";
    }
    ?>
